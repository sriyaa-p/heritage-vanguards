...
export default function ConfidenceCardPage() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const data = await res.json();
        setStatus(data.status);
        if (["reviewer_review", "approved", "rejected"].includes(data.status)) {
          setDossier(data.dossier);
  ...
  async function handleDecision(decision: "committee_review" | "rejected") {
    if (!notes.trim()) {
      alert("Please enter reviewer notes explaining your decision.");
      return;
    }
    setDeciding(true);
    try {
    ...
    return (
      <main className="min-h-screen bg-gray-50 flex items-center justify-center p-8">
        <div className="bg-white rounded-xl shadow p-8 max-w-md w-full text-center">
          <div className="text-5xl mb-4">{finalDecision === "committee_review" ? "📤" : "❌"}</div>
          <h2 className="text-2xl font-bold text-gray-900 mb-2">
            {finalDecision === "committee_review" ? "Recommended" : "Rejected"}
          </h2>
          <p className="text-gray-500">
            {dossier.metadata?.location_name} has been {finalDecision === "committee_review" ? "recommended and forwarded to the committee" : "rejected"}.
          </p>
  ...
  const alreadyDecided = ["committee_review", "approved", "rejected"].includes(status);
      ...
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        {alreadyDecided ? (
          <div className="bg-white rounded-xl shadow p-6 text-center">
            <p className="text-sm text-gray-500">This submission has already been <span className="font-semibold capitalize">{status === "committee_review" ? "Recommended" : status}</span>.</p>
            {dossier.review?.reviewer_notes && (
              <div className="mt-3 p-3 bg-gray-50 rounded text-left text-xs text-gray-600 italic">
                "{dossier.review.reviewer_notes}"
              </div>
            )}
            <button onClick={() => router.push("/review")} className="mt-4 text-blue-600 hover:underline text-sm">← Back to review queue</button>
          </div>
        ) : (
          <div className="bg-white rounded-xl shadow p-6">
            ...
            <textarea
              ...
              rows={3}
              placeholder="Enter evaluation notes (required)..."
              value={notes}
            ...
            <div className="flex gap-3">
              <button
                onClick={() => handleDecision("committee_review")}
                disabled={deciding}
              ...
              >
                {deciding ? "Saving…" : "Recommend & Forward"}
              </button>
              <button
